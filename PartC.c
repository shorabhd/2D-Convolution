...
int main(int argc, char* argv[])
{
	...
	/*1D FFT Row wise*/
	#pragma omp parallel num_threads(2) shared(data1,data2)
	...
	#pragma omp parallel num_threads(2) shared(data1,data2,data3)
	#pragma omp for private(i,j)
	for (i = 0; i < chunk; i++)
	{
		for (j = 0; j < MAXN; j++)
	...
	#pragma omp parallel num_threads(2) shared(data3)
	#pragma omp for private(i)
	for (i = 0; i<chunk; i++)
	{
